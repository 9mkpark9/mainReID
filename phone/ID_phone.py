...
def detect_phone_pose_based(keypoints, threshold_face=80, min_distance_between_hands=40):
    ...
    # 얼굴 근처에 손이 있어야 하며, 양손이 너무 가깝지 않아야 함
        ...
        while cap.isOpened():
            ...
            for result in pose_results:
                for bbox in result.boxes.xyxy:
                    ...
                    w = x2 - x1
                    h = y2 - y1
                    detections.append(([x1, y1, w, h], 1.0, 0))  # (bbox, confidence, class)
            ...
            for track in tracks:
                ...
                for result in pose_results:
                    for bbox, keypoints in zip(result.boxes.xyxy, result.keypoints):
                        bbox_x1, bbox_y1, bbox_x2, bbox_y2 = map(int, bbox.cpu().numpy())
                        if abs(bbox_x1 - x1) < 20 and abs(bbox_y1 - y1) < 20:  # 같은 사람인지 확인
                            ...
                            # ID 표시
                            cv2.putText(frame, f"ID: {final_id}", (x1, y1 - 30),
